/* @fn new Threefunc(t,a)
 * @brief initialize a new object for three three 3-ary Psi by t and a.
...
Threefunc = function(t,a){
  ...
  switch(t){
    ...
    case "1":
      this.t=",";
      this.a=[new Threefunc("0"), new Threefunc("0"), new Threefunc("0")];
    return;
    case "w":
      this.t=",";
      this.a=[new Threefunc("0"), new Threefunc("0"), new Threefunc("1")];
    return;
    case "W":
      this.t=",";
      this.a=[new Threefunc("0"), new Threefunc("1"), new Threefunc("0")];
    return;
    case "e":
      this.t=",";
      this.a=[new Threefunc("0"), new Threefunc("0"), new Threefunc("W")];
    return;
    case "z":
      var k=Threefunc.parse("(0,0,(0,1,W))");
      this.t=k.t;
      this.a=k.a;
    return;
    case "G":
      var k=Threefunc.parse("(0,0,(0,1,(0,1,W)))");
      this.t=k.t;
      this.a=k.a;
    return;
    default:
...
/* @fn Threefunc.toSugar
 * @brief Callback for sugar for toString(sugar). 
 * @detail When you use three.toString(), if you add the function for the parameter of toString() like three.toString(Threefunc.toSugar), toString will become to use the sugar syntax defined in the function. 
 * @param str = input string from toString().
 * @returns str = modified string which is finally output by toString(). */
Threefunc.toSugar = function(str){
...
/* programmer memo: Please put the code as following to make the type Threefunc the extension of the type Ordinal. */
/* ------------------0---------------------------------- begin */
Threefunc.prototype = Object.create(Ordinal.prototype);
Object.defineProperty(Threefunc.prototype, 'constructor', 
  {value:Threefunc, enumerable:false, writable:true});
Threefunc.parse     = Ordinal.parse;
Threefunc.add       = Ordinal.add;
Threefunc.cat       = Ordinal.cat;
/* ----------------------------------------------------- end */

Threefunc.prototype.normalize = function(){
  Ordinal.prototype.normalize();
  if(this.t==","){
    if(this.a.length==1) this.a=[].concat([Threefunc.k0,Threefunc.k0],this.a);
    if(this.a.length==2) this.a=[].concat([Threefunc.k0            ],this.a);
  }
}
Threefunc.prototype.iszero=function(){return this.eq(k0);}
Threefunc.prototype.isone =function(){return this.eq(k1);}
Threefunc.prototype.isw   =function(){return this.eq(kw);}
Threefunc.prototype.isPT  =function(){return this.t==",";}
Threefunc.prototype.isadd =function(){return this.t=="+";}
Threefunc.prototype.slice =function(s,e){ return new Threefunc(this.t, this.a.slice(s,e));}
Threefunc.prototype.eq    =function(x){return Threefunc.eq(this,x);}
Threefunc.prototype.lt    =function(x){return Threefunc.lt(this,x);}
Threefunc.prototype.isfinite=function(){
...
Threefunc.prototype.toint=function(){
  switch(this.t){
    case "0": return 0;
...
Threefunc.prototype.mul=function(n){
  if(n==0)return Threefunc.k0;
  if(n==1)return this;
  var a=new Array(n);
  for(var i=0;i<a.length;i++) a[i]=this;
  return new Threefunc("+",a);
}

Threefunc.eq=function(x,y){
  if(x.t!=y.t) return false;
  if(x.a instanceof Array && y.a instanceof Array){
    if(x.a.length!=y.a.length) return false;
    for(var i=0;i<x.a.length;i++){
      if(!Threefunc.eq(x.a[i],y.a[i])) return false;
    }
  }
  return true;
}
Threefunc.k0=new Threefunc("0");
Threefunc.k1=new Threefunc("1");
Threefunc.kw=new Threefunc("w");
Threefunc.kW=new Threefunc("W");
/* original part -------------------------------------------------------------------------------*/

/** @fn lt(x,y)
  * @brief compare x and y and returns ordering of them.  
  * @param x = Threefunc ordinal notation.
  * @param y = Threefunc ordinal notation.
...
Threefunc.lt=function(X,Y){
  if(!X instanceof Threefunc) throw new Error("X is not Threefunc object.");
  if(!Y instanceof Threefunc) throw new Error("Y is not Threefunc object.");
  var eq = Threefunc.eq;
  var lt = Threefunc.lt;
...
/* 共終数 */
/* ここでは、共終数という概念を定義する。 */
Threefunc.prototype.dom=function(){
...
Threefunc.prototype.expand=function(Y){
  if(!Y instanceof Threefunc) throw new Error("Y is not Threefunc object.");
  var dom = function(A){return A.dom();};
  var lt = Threefunc.lt;
  var k0 = Threefunc.k0;
  var kw = Threefunc.kw;
  var X  = this;
  var newk = function(X_1,X_2,X_3){return new Threefunc(",",[X_1,X_2,X_3]);};
  /* 1.            もしX=0ならば、    X[Y]=0である。 */
...
Threefunc.prototype.countzero=function(){
  var retval=0;
...
Threefunc.prototype.isstd=function(){
  var X=this;
  var lt=Threefunc.lt;
  var lt=Threefunc.eq;
  if(X.iszero())return true;
    ...
    var S;
    while(1){
      S=new Threefunc(",",[0,0,C]);
      if(lt(X,S)){
        break;
      }
      C=new Threefunc(",",[C,0,0]); //upgrade C
    }
